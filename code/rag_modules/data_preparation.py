...
class DataPreparationModule:
    ...
    def load_documents(self) -> List[Document]:
        ...
        Returns:
            加载的文档列表
        """
        logger.info(f"正在从 {self.data_path} 加载文档...")  # 日志
        ...
        for doc in documents:
            ...
            # 缓存父文档，方便之后查找
            save_parent_id = doc.metadata.get("parent_id")
            if save_parent_id:
                self.parent_id_to_doc[save_parent_id] = doc

        self.documents = documents
        logger.info(f"加载成功{len(documents)}个文档")
        return documents
    ...
    def _enhance_metadata(self, doc: Document):
        ...
        path_parts = (
            file_path.parts
        )  # 拆分路径，比如：notes/chapter1/intro.md -> ('notes', 'chapter1', 'intro.md')
        ...
        # 提取菜品名称
        doc.metadata["dish_name"] = file_path.stem  # stem：路径末
    ...
    def chunk_documents(self) -> List[Document]:
        ...
        for index, chunk in enumerate(chunks):
            if "chunk_id" not in chunk.metadata:
                # 如果没有这个id(比如分割失败)，就生成一个
                chunk.metadata["chunk_id"] = str(uuid.uuid4())
            chunk.metadata["batch_index"] = index  # 在全局所有块的大列表的索引
    ...
    def _markdown_header_split(self) -> List[Document]:
        ...
        for doc in self.documents:
            try:
                ...
                if not has_headers:
                    logger.warning(
                        f"文档 {doc.metadata.get('dish_name', '未知')} 内容中没有发现Markdown标题"
                    )
                ...
                logger.debug(
                    f"文档 {doc.metadata.get('dish_name', '未知')} 分割成 {len(md_chunks)} 个chunk"
                )

                # 如果没有分割成功，说明文档可能没有标题结构
                if len(md_chunks) <= 1:
                    logger.warning(
                        f"文档 {doc.metadata.get('dish_name', '未知')} 未能按标题分割，可能缺少标题结构"
                    )
                ...
                all_chunks.extend(
                    md_chunks
                )  # 把当前文档切出来的所有文本块，平铺添加到总块列表里
            except Exception as e:
                logger.warning(
                    f"文档 {doc.metadata.get('source', '未知')} Markdown分割失败: {e}"
                )
    ...
    def filter_documents_by_category(self, category: str) -> List[Document]:
        ...
        """
        return [
            doc for doc in self.documents if doc.metadata.get("category") == category
        ]
    ...
    def get_parent_documents(self, child_chunks: List[Document]) -> List[Document]:
        ...
        for parent_id in sorted_parent_ids:
            # 无需判断是否存在，所有父文档id都在字典里
            parent_docs.append(self.parent_id_to_doc[parent_id])

        # parent_docs = [self.parent_id_to_doc[pid] for pid in sorted_parent_ids] 简化写法
        # 收集父文档名称和相关性信息用于日志
        parent_info = []
